...
export default function SmartStrokeDashboard({ classId, onSaveSuccess, role = 'teacher', socket }) {
  ...
  useEffect(() => {
    ...
    const handleReceiveStroke = (s) => {
      if (s.pageIndex !== currentPageIndex) return;
    ...
    const handleReceiveAction = (act) => {
      if (act.action === 'clear') {
        ...
        // Pass null to indicate the session ended without saving a new file
        // This prevents the "Note Archived" toast in App.jsx
        onSaveSuccess?.(null); 
      }
    };
  ...
  }, [isStudent, socket, currentPageIndex]);
  ...
  const clearCurrentPage = () => {
    ...
    canvasRef.current?.getContext('2d')?.clearRect(0, 0, 1400, 700);
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const occupiedWidth = isStudent ? 0 : (isCompact ? 40 : 280); 
      const occupiedHeight = isStudent ? 0 : (isCompact ? 180 : 80); 
      const availableWidth = window.innerWidth - occupiedWidth;
      const availableHeight = window.innerHeight - occupiedHeight; 
      setBaseScale(Math.min(availableWidth / 1400, availableHeight / 700, 1.2)); 
    };
  ...
  useEffect(() => {
    ...
    const targetX = 1400 / 2 - (yaw - centerPos.current.yaw) * SENSITIVITY;
    const targetY = 700 / 2 - (pitch - centerPos.current.pitch) * SENSITIVITY;
    ...
    x = Math.max(0, Math.min(x, 1400)); y = Math.max(0, Math.min(y, 700));
  ...
  }, [data, selectedColor, isConnected, isStudent, classId, socket, currentPageIndex]);

  // --- STREAM START ---
  useEffect(() => {
    if (!isStudent && socket) {
      socket.emit('start-stream', classId);
      return () => { socket.emit('stop-stream', classId); };
    }
  ...
  const renderStrokesToCtx = (ctx, strokesList) => {
    ...
    ctx.clearRect(0, 0, 1400, 700); ctx.lineCap = 'round'; ctx.lineJoin = 'round'; ctx.lineWidth = 4;
  ...
  // Inside SmartStrokeDashboard / IMUCanvas.jsx
  useEffect(() => {
    if (!isStudent && socket) {
      ...
      return () => {
        // ✅ Signal to everyone in the room that the session has ended
  ...
  const resetSession = () => { setPages([[]]); setCurrentPageIndex(0); currentStrokePoints.current = []; prevCoords.current = null; renderStrokesToCtx(canvasRef.current?.getContext('2d'), []); setZoomScale(1); setOffset({ x: 0, y: 0 }); };
  ...
  const finalizeDownload = async () => {
    ...
    try {
      ...
      if (res.data.message === "Success") { onSaveSuccess?.(res.data.file); setShowPreview(false); URL.revokeObjectURL(pdfBlobUrl); resetSession(); }
    ...
    try {
      ...
      characteristic.addEventListener('characteristicvaluechanged', (e) => {
        try {
          ...
          if (p.pdf === 1 || (p.np === 1 && lastNpSignal.current === 0)) { if (p.pdf === 1) generatePreview(); else createNewPage(); }
          lastNpSignal.current = p.np || 0;
          setData({ ...p, down: p.d === 1 || p.d === true });
        } catch {}
      });
      bleDevice.current = device; setIsConnected(true); triggerToast("Pen Connected");
    } catch { triggerToast("Connection failed"); }
  };

  const disconnectBLE = () => { if (bleDevice.current?.gatt.connected) { bleDevice.current.gatt.disconnect(); setIsConnected(false); triggerToast("Pen Disconnected"); } };
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen w-screen bg-slate-100 overflow-hidden font-['Poppins'] select-none">
        ...
        <aside className="w-full lg:w-[240px] bg-white p-3 lg:p-5 flex flex-row lg:flex-col border-b lg:border-r border-slate-200 z-50 overflow-x-auto lg:overflow-y-auto no-scrollbar gap-4 lg:gap-0 shrink-0">
          ...
          <div className="flex lg:flex-col gap-2 shrink-0">
            {!isConnected ? (
              ...
              <><button onClick={disconnectBLE} className="whitespace-nowrap p-2 lg:p-2.5 rounded-xl border border-slate-200 bg-white text-slate-500 font-bold cursor-pointer text-xs lg:text-sm">Disconnect</button><button onClick={handleRecenter} className="whitespace-nowrap p-2 lg:p-2.5 rounded-xl bg-blue-100 text-blue-700 font-bold cursor-pointer text-xs lg:text-sm">Recenter</button></>
            )}
      ...
      <main ref={mainRef} className={`flex-1 relative overflow-hidden bg-slate-200 p-2 md:p-4 touch-none ${isStudent ? 'cursor-default' : 'cursor-grab active:cursor-grabbing'}`} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleEnd} onMouseLeave={handleEnd} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleEnd}>
        <div className="absolute origin-center transition-transform duration-75 ease-out" style={{ left: '50%', top: '50%', width: '1400px', height: '700px', transform: `translate(-50%, -50%) translate(${offset.x}px, ${offset.y}px) scale(${baseScale * zoomScale})`, flexShrink: 0 }}>
          <div className="relative w-full h-full rounded-lg shadow-2xl border-[12px] border-blue-900 bg-white">
            ...
            {!isStudent && <canvas ref={cursorRef} width={1400} height={700} className="absolute top-0 left-0 pointer-events-none" />}
          </div>
        </div>
        {(zoomScale !== 1 || offset.x !== 0 || offset.y !== 0) && (<button onClick={() => { setZoomScale(1); setOffset({x:0, y:0}); }} className="absolute bottom-6 left-6 bg-white shadow-xl px-4 py-2 rounded-full text-[10px] font-black text-blue-900 border border-blue-100 uppercase tracking-widest z-[100]">Reset View</button>)}
      </main>

      {toast.show && !isStudent && (<div className="fixed bottom-20 md:bottom-8 left-1/2 -translate-x-1/2 bg-zinc-700 text-white px-6 py-3 rounded-xl shadow-lg text-xs md:text-sm font-semibold z-[2000] animate-bounce">{toast.message}</div>)}
        ...
        <div className="fixed inset-0 bg-slate-900/85 backdrop-blur-sm flex items-center justify-center z-[1000] p-4">
          <div className="bg-white rounded-[24px] w-full max-w-[1100px] max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden p-4 md:p-6 gap-4">
              {/* ✅ FIXED: Styled iframe container with white background to prevent black flash */}
              <div className="flex-1 bg-white rounded-2xl p-2 md:p-3 flex min-h-[300px]">
                <iframe src={pdfBlobUrl} className="w-full h-full border-none rounded-lg bg-white" title="PDF Preview" />
              </div>
              {!isStudent && (
