...
export default function ClassDetail({ user, classroom, onBack, onStartSession, triggerToast }) {
  ...
  const handleConfirmLeave = async () => {
    ...
    if (!idToSend) {
      triggerToast("Error: User session not found", "error");
      return;
    }
  ...
  return (
    <div className="relative min-h-screen animate-in slide-in-from-bottom-4 duration-700 pb-0">
      ...
      <div className="relative z-10 px-4 md:px-0">
        ...
        {/* Header Card */}
        <div className="bg-white/70 backdrop-blur-xl p-6 md:p-10 rounded-[32px] md:rounded-[48px] shadow-2xl border border-white mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div className="flex-1 w-full md:w-auto">
              ...
              <h2 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tighter mb-4 break-words">{classroom?.name}</h2>
              
              <div className="flex flex-col gap-4">
                {user?.role === 'teacher' ? (
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-2">
                      <span className="bg-orange-50 text-[#FF8040] px-4 md:px-6 py-2 rounded-2xl font-mono font-black text-xl md:text-2xl border border-orange-100 shadow-sm">{classroom?.code}</span>
                      <button onClick={handleCopy} className="p-3 bg-white border border-slate-100 rounded-2xl text-slate-400 hover:text-[#001BB7] transition-all active:scale-90">
          ...
          {user?.role === 'teacher' ? (
              <button onClick={onStartSession} className="w-full md:w-auto bg-[#001BB7] text-white px-10 py-6 rounded-[24px] md:rounded-[28px] font-black shadow-2xl hover:bg-[#0046FF] transition-all flex items-center justify-center gap-3 uppercase text-xs tracking-[0.2em] active:scale-95">
              ...
              <button 
                ...
                className={`w-full md:w-auto px-10 py-6 rounded-[24px] md:rounded-[28px] font-black shadow-2xl transition-all flex items-center justify-center gap-3 uppercase text-xs tracking-[0.2em] 
                  ${isLive 
                    ? 'bg-[#FF8040] text-white hover:bg-[#e66a2e] animate-pulse cursor-pointer active:scale-95' 
        ...
        {/* Notes and Students Grid */}
        <div className={`grid grid-cols-1 ${user?.role === 'teacher' ? 'lg:grid-cols-3' : ''} gap-8 mb-10`}>
            <div className={`${user?.role === 'teacher' ? 'lg:col-span-2' : ''} bg-white/60 backdrop-blur-md rounded-[32px] md:rounded-[40px] border border-white overflow-hidden shadow-xl`}>
                <div className="p-6 md:p-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/40">
                    <div>
                    ...
                    <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-xl w-full sm:w-auto">
                        <button onClick={() => toggleSort('time')} className={`flex-1 sm:flex-none px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${sortBy === 'time' ? 'bg-white text-[#001BB7] shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>Time {sortBy === 'time' && (sortOrder === 'asc' ? '↑' : '↓')}</button>
                        <button onClick={() => toggleSort('name')} className={`flex-1 sm:flex-none px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${sortBy === 'name' ? 'bg-white text-[#001BB7] shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>Name {sortBy === 'name' && (sortOrder === 'asc' ? '↑' : '↓')}</button>
                    </div>
                </div>
                <div className="p-4 space-y-4 max-h-[400px] overflow-y-auto no-scrollbar">
                    {sortedFiles?.length > 0 ? sortedFiles.map((file, idx) => (
                        <div key={file._id || idx} className="group flex flex-col sm:flex-row justify-between items-center p-5 md:p-6 bg-white/80 border border-slate-100/50 rounded-[24px] md:rounded-[32px] hover:border-[#001BB7] hover:shadow-2xl transition-all duration-500 gap-4">
                            <div className="flex items-center gap-4 md:gap-5 overflow-hidden w-full sm:w-auto">
                                <div className="bg-blue-50 p-3 md:p-4 rounded-2xl text-[#001BB7] group-hover:bg-[#001BB7] group-hover:text-white transition-all">
                    ...
                    )) : <p className="text-center text-slate-300 py-10 font-bold">No archives yet.</p>}
                </div>
            </div>

            {user?.role === 'teacher' && (
                <div className="bg-white/60 backdrop-blur-md rounded-[32px] md:rounded-[40px] border border-white overflow-hidden shadow-xl flex flex-col">
                    ...
                    <div className="p-6 space-y-3 max-h-[400px] overflow-y-auto no-scrollbar flex-1">
                        {students.map((s) => {
      ...
      {showLeaveConfirm && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 animate-in fade-in duration-300">
          ...
          <div className="relative bg-white p-8 md:p-10 rounded-[32px] md:rounded-[48px] shadow-2xl w-full max-w-md text-center border border-white animate-in zoom-in-95">
              ...
              <h3 className="text-2xl font-black text-slate-900 mb-2 uppercase tracking-tight">Leave Classroom?</h3>
              ...
              <div className="flex flex-col gap-3">
                  <button disabled={isProcessing} onClick={handleConfirmLeave} className="w-full bg-red-500 text-white py-5 rounded-[20px] md:rounded-[24px] font-black shadow-xl hover:bg-red-600 transition-all active:scale-95 uppercase text-xs tracking-widest disabled:opacity-50">
                      {isProcessing ? "Processing..." : "Confirm Leave"}
                  </button>
                  <button disabled={isProcessing} onClick={() => setShowLeaveConfirm(false)} className="w-full bg-slate-50 text-slate-400 py-5 rounded-[20px] md:rounded-[24px] font-black hover:bg-slate-100 transition-all uppercase text-xs tracking-widest">
                      Cancel
                  </button>
              </div>
          </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 animate-in fade-in duration-300">
          ...
          <div className="relative bg-white p-8 md:p-10 rounded-[32px] md:rounded-[48px] shadow-2xl w-full max-w-md text-center border border-white animate-in zoom-in-95">
              ...
              <h3 className="text-2xl font-black text-slate-900 mb-2 uppercase tracking-tight">Delete Classroom?</h3>
              <p className="text-slate-500 text-sm mb-10 leading-relaxed font-medium">
              ...
              <div className="flex flex-col gap-3">
                  <button disabled={isProcessing} onClick={handleConfirmDelete} className="w-full bg-red-500 text-white py-5 rounded-[20px] md:rounded-[24px] font-black shadow-xl hover:bg-red-600 transition-all active:scale-95 uppercase text-xs tracking-widest disabled:opacity-50">
      ...
      {showQRModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 animate-in fade-in duration-300">
            ...
            <div className="relative bg-white p-8 md:p-10 rounded-[32px] md:rounded-[48px] shadow-2xl w-full max-w-sm text-center border border-white animate-in zoom-in-95">
